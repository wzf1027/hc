...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, 
        ...
        try {
			...
			if(redisUtil.get(obj.toString())== null ||  !(redisUtil.get(obj.toString()) instanceof Seller)) {
				response.setContentType("application/json; charset=utf-8");
				response.setStatus(403);
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("code", 403);
			...
			Seller seller = (Seller) redisUtil.get(obj.toString());
			seller = sellerMapper.findSellerbyId(seller.getSellerId());
			if(seller!=null&&seller.getEnabled().equals(1))
			{
				response.setContentType("application/json; charset=utf-8");
				response.setStatus(403);
				Map<String, Object> map = new HashMap<String, Object>();
			...
			redisUtil.set(obj.toString(), seller, Constant.SAVEUSERTIME);
			redisUtil.set("ONLINE_"+seller.getSellerId(), new Date().getTime()+"",Constant.ONLINETIME);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
