...
public class GlobalExceptionHandler {
    ...
    public String credentials(CredentialsException e, Model model) {
        String username = getRequest().getParameter("username");
        LogManager.me().executeLog(LogTaskFactory.loginLog(username,"账号密码", getIp()));
        model.addAttribute("tips","账号密码");
        return "/login.html";
    }

    @ExceptionHandler(GoogleCodeException.class)
    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    public String GoogleCodeException(GoogleCodeException e, Model model) {
        String username = getRequest().getParameter("username");
        LogManager.me().executeLog(LogTaskFactory.loginLog(username, e.getMessage(), getIp()));
        model.addAttribute("tips","输入的谷歌验证码有误");
        return "/login.html";
    }
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    @ResponseBody
    public cn.stylefeng.guns.core.util.ResponseData MemberLoginException(MemberLoginException e) {
    	   log.error("MemberLoginException异常!", e);
        return  cn.stylefeng.guns.core.util.ResponseData.error(e.getMessage());
    }
    
    @ExceptionHandler(WallterException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    @ResponseBody
    public cn.stylefeng.guns.core.util.ResponseData WallterException(WallterException e) {
    	   log.error("WallterException异常!", e);
        return  cn.stylefeng.guns.core.util.ResponseData.error(e.getMessage());
    }
}
