...
public class SystemController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("/examineCheck")
    public String examineCheck() {
    	  ShiroUser user = ShiroKit.getUserNotNull();
    	  if(user.getIsAuth()==0) {
    		  return "/modular/frame/examine_check.html";
    	  }
		  return "/modular/frame/examine_success.html";
    }

    @RequestMapping("/examineSuccessPage")
    public String examineSuccessPage() {
        return "/modular/frame/examine_success.html";
    }
    ...
    @ResponseBody
    public ResponseData getMsg(){
    	User user = userService.getById(ShiroKit.getUser().getId());
    	if(user!= null) {
    		if(StringUtils.isBlank(user.getPhone())) {
    			return ResponseData.error("请去个人中心设置手机号码");
    		}
    		int number =123456;
    		if("1".equals(isSwitch)) {
        		redisUtil.set(Constant.SMS+user.getPhone(),String.valueOf(number));
	    	 ...
	    	 number = (int) ((Math.random()*9+1)*100000);
	           ...
	           if(sendSMSExtService.sendSms(content,user.getPhone(),1L)){
	            	redisUtil.set(Constant.SMS+user.getPhone(),String.valueOf(number),120);
	            	return ResponseData.success(200, "发送成功", null);
	            }
